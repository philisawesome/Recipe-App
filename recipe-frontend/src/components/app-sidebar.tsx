...
function MenuLinksComponent(props: { auth: any; closeMenu?: () => any }) {
  ...
  return (
    <SidebarMenu className="w-fit">
      {MenuLinks.filter((s: MenuLink) => auth.loggedIn || !s.auth).map(
        (s: MenuLink, i: number) => {
          return (
            <SidebarMenuItem key={i}>
              <SidebarMenuButton asChild>
                <a href={s.link}>
                  <span className="text-xl md:text-md">{s.name}</span>
                </a>
              </SidebarMenuButton>
            </SidebarMenuItem>
          );
...
function ProfileSection(props: { auth: any }) {
  ...
  return (
    <div className="w-fit lg:w-full gap-3 p-4 flex flex-col">
      {auth.loggedIn && (
        <div>
          <AvatarCard user={auth.user} />
          <Button asChild className="bg-(--color-4) w-full">
...
function Logo() {
  return (
    <a
      href="/index"
      className="flex pt-2 pl-2 group-data-[collapsible=icon]:p-0"
    >
      <div
...
export function AppSidebar() {
  ...
  useEffect(() => {
    ...
    loggedIn.subscribe((l) => setIsLoggedIn(l));
  }, []);
  const auth = { user: { avatar, username }, loggedIn: isLoggedIn };
  return (
    <Sidebar
      collapsible="icon"
      className="flex flex-col overflow-hidden
			justify-between invisible h-[100vh] lg:visible"
      side="left"
    >
      <SidebarHeader>
        <Logo />
...
export function MobileSidebar(props: { open: boolean; setOpen: any }) {
  ...
  return (
    <div className="visible lg:invisible">
      ...
      {open && (
        <div className="fixed w-[100vw] h-[100vh] top-0 left-0 p-5 pt-9 flex flex-col bg-white">
          <div className="flex flex-col items-center h-[100vh]">
            <Logo />
            <MenuLinksComponent auth={auth} closeMenu={() => setOpen(false)} />
            <ProfileSection auth={auth} />
          </div>
        </div>
      )}
    ...
    <div>
      ...
      <div className="visible lg:invisible absolute top-0 left-0 bg-white z-5">
        <Button
          ...
          className="text-lg"
        >
          <img src="/menu-dots.svg" alt="show menu" className="w-[3rem]" />
        </Button>
      </div>
    </div>
  );
